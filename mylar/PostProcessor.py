...
#        for curScriptName in mylar.CONFIG.EXTRA_SCRIPTS:
        ...
        if tinfo is not None: #manual is None:
            ...
            if all([sandwich is not None, helpers.is_number(sandwich), sarc is None]):
                ...
                if any([oneoff is True, issuenzb is None]) or all([sandwich is not None, 'S' in str(sandwich), oneoff is True]) or int(sandwich) >= 900000:
                    ...
                    if any([all([mylar.CONFIG.PUSHOVER_IMAGE, mylar.CONFIG.PUSHOVER_ENABLED]), all([mylar.CONFIG.TELEGRAM_IMAGE, mylar.CONFIG.TELEGRAM_ENABLED])],mylar.CONFIG.MATTERMOST_ENABLED):
                        try:
                            get_cover = getimage.extract_image(grab_dst, single=True, imquality='notif')
                            imageFile = get_cover['ComicImage']
                        except Exception as e:
    ...
    def Process_next(self, comicid, issueid, issuenumOG, ml=None, stat=None):
            ...
            if any([all([mylar.CONFIG.PUSHOVER_IMAGE, mylar.CONFIG.PUSHOVER_ENABLED]), all([mylar.CONFIG.TELEGRAM_IMAGE, mylar.CONFIG.TELEGRAM_ENABLED]), all([mylar.CONFIG.DISCORD_ENABLED]),all([mylar.CONFIG.MATTERMOST_ENABLED])]):
                try:
    ...
    def sendnotify(self, series, issueyear, issuenumOG, annchk, module, imageFile):
        ...
        try:
            ...
            if mylar.CONFIG.MATTERMOST_ENABLED:
                meta = { 'series': series, 'issue': issuenumOG, 'year': issueyear}
                mattermost = notifiers.MATTERMOST()
                mattermost.notify("Downloading and Postprocessing completed", prline2,imageFile=imageFile, metadata=meta, module=module)
