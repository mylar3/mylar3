                        ...
                        if AlternateSearch is not None and AlternateSearch != "None":
                            ...
                            for calt in chkthealt:
                                ...
                                findit = NZB_SEARCH(AS_Alternate, IssueNumber, ComicYear, SeriesYear, Publisher,
                                                    IssueDate, StoreDate, searchprov, send_prov_count, IssDateFix,
                                                    IssueID, UseFuzzy, newznab_host, ComicVersion=ComicVersion,
                                                    SARC=SARC, IssueArcID=IssueArcID, RSS="yes", ComicID=ComicID,
                                                    issuetitle=issuetitle, unaltered_ComicName=AS_Alternate,
                        ...
                        if AlternateSearch is not None and AlternateSearch != "None":
                            ...
                            for calt in chkthealt:
                                ...
                                findit = NZB_SEARCH(AS_Alternate, IssueNumber, ComicYear, SeriesYear, Publisher,
                                                    ...
                                                    issuetitle=issuetitle, unaltered_ComicName=unaltered_ComicName,
                                                    allow_packs=allow_packs, oneoff=oneoff, cmloopit=cmloopit,
...
def NZB_SEARCH(ComicName, IssueNumber, ComicYear, SeriesYear, Publisher, IssueDate, StoreDate, nzbprov, prov_count,
               IssDateFix, IssueID, UseFuzzy, newznab_host=None, ComicVersion=None, SARC=None, IssueArcID=None,
               RSS=None, ComicID=None, issuetitle=None, unaltered_ComicName=None, allow_packs=None, oneoff=False,
    ...
    cm = re.sub("\\band\\b", "",
                cm1.lower())  # remove 'and' & '&' from the search pattern entirely (broader results, will filter out later)
        ...
        # this is the API calls
            ...
            if nzbprov == '32P':
                ...
                if nzbprov == 'dognzb':
                    ...
                    logger.warn(
                        'You have a blank newznab entry within your configuration. Remove it, save the config and restart mylar to fix things. Skipping this blank provider until fixed.')
                    findurl = None
                    bb = "noresults"
        ...
        if not bb == "no results":
            for entry in bb['entries']:
                ...
                if parsed_comic['parse_status'] == 'success' and (
                    ...
                    try:
                        ...
                        if filecomic['process_status'] == 'fail':
                            ...
                            logger.fdebug(
                                '%s was a match due to alternate matching.  Continuing to search, but retaining this result just in case.' % ComicTitle)
                            alt_match = True
                            ...
                            if IssDateFix != "no" and UseFuzzy != "2":
                                if IssDateFix == "01" or IssDateFix == "02" or IssDateFix == "03":
                                    ...
                                    if str(ComicYearFix) in result_comyear:
                                        logger.fdebug(
                                            'Further analysis reveals this was published inbetween Nov-Jan, decreasing year to %s has resulted in a match!' % ComicYearFix)
                                        yearmatch = "true"
                                    else:
                    ...
                    if F_ComicVersion == '0':
                        ...
                        if postdate_int is not None:
                            if digitaldate != '0000-00-00' and all([postdate_int >= digitaldate_int, nzbprov == '32P']):
                                logger.fdebug(
                                    '32P torrent discovery. Posting date (%s) is after DIGITAL store date (%s), forcing volume label to be the same as series label (0-Day Enforcement): v%s --> v%s' % (
                ...
                if downloadit:
                    ...
                    if nzbname is None:
                        logger.error(
                            '[NZBPROVIDER = NONE] Encountered an error using given provider with requested information: %s. You have a blank entry most likely in your newznabs, fix it & restart Mylar' % mylar.COMICINFO)
                        continue
                    ...
                    searchresult = searcher(nzbprov, nzbname, mylar.COMICINFO, links, IssueID, ComicID, tmpprov,
                                            newznab=newznab_host, torznab=torznab_host, rss=RSS)
    ...
    if foundc['status'] is True:
        ...
        if mylar.COMICINFO[0]['pack'] is True:
            ...
            if issinfo is not None:
                ...
                try:
                    logger.fdebug(
                        'Found matching comic within pack...preparing to send to Updater with IssueIDs: %s and nzbname of %s' % (
    ...
    try:
        ...
        if (any([mylar.CONFIG.ENABLE_DDL is True, mylar.CONFIG.NZBSU is True, mylar.CONFIG.DOGNZB is True,
            ...
            if not issueid or rsscheck:
                ...
                if rsscheck:
                    logger.info(
                        'Initiating RSS Search Scan at the scheduled interval of %s minutes' % mylar.CONFIG.RSS_CHECKINTERVAL)
    ...
    if (any([mylar.CONFIG.ENABLE_DDL is True, mylar.CONFIG.NZBSU is True, mylar.CONFIG.DOGNZB is True,
        ...
        for issueid in issuelist:
            ...
            if issue is None:
                ...
                if issue is None:
                    logger.warn(
                        'Unable to determine IssueID - perhaps you need to delete/refresh series? Skipping this entry: %s' % issueid)
                    continue

            if any([issue['Status'] == 'Downloaded', issue['Status'] == 'Snatched']):
                logger.fdebug(
                    'Issue is already in a Downloaded / Snatched status. If this is still wanted, perform a Manual search or mark issue as Skipped or Wanted.')
                continue
...
def searcher(nzbprov, nzbname, comicinfo, link, IssueID, ComicID, tmpprov, directsend=None, newznab=None, torznab=None,
    ...
    if mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
        ...
        if all([nzbid is not None and nzbid != '', IssueID is not None, oneoff is False]):
            ...
            logger.fdebug(
                '[FAILED_DOWNLOAD_CHECKER] Failed download checking is not available for one-off downloads atm. Fixed soon!')
    ...
    # torrents (32P & DEM)
        ...
        if rcheck == "fail":
            if mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
                ...
                logger.error(
                    'Unable to send torrent - check logs and settings (this would be marked as a BAD torrent if Failed Handling was enabled)')
                return "torrent-fail"
        else:
    ...
    if directsend is None:
        ...
        if alt_nzbname is None or alt_nzbname == '':
            ...
            logger.fdebug(
                "Found matching comic...preparing to send to Updater with IssueID %s and nzbname of %s [ALTNZBNAME:%s][OneOff:%s]" % (
