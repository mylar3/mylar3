    ...
    while True:
        if mylar.DDL_LOCK is True:
            ...
            if ddzstat['success'] and ddzstat['filename'] is not None:
                filecondition = checkFileCondition(ddzstat['path'])
                if not filecondition['status']:
                    logger.warn(f"Downloaded file has quality issues ({filecondition['quality']}).  Marking as failed.")
            ...
            if gumbo_line:
                ...
                if mylar.SEARCHLOCK is False:
                    ...
                    if local_check['status']:
                        ...
                        filecondition = checkFileCondition(fullpath)
                        if not filecondition['status']:
                            logger.warn(f"File found in folder cache has quality issues ({filecondition['quality']}).  Ignoring.")
...
def checkFileCondition(filePath):
    """ Use magic numbers to confirm a file type, and do some sanity checks for quality of
        the file (CRC checks, etc.)

    Args:
        filePath (str): Location of the file to check

    Returns:
        dict: A dictionary containing a status (True/False for good/bad file), type (known file type), and quality (descriptive string)
    """
    logger.fdebug(f'Checking file condition of {filePath}')
    ...
    try:
        ...
        logger.error(f"Could not open {filePath} to check for file type")
        return {'status': False, 'type' : 'unknown', 'quality': 'Failed to open file to check quality'}
    ...
    if header.startswith(magic_numbers['ZIP']):
        try:
            ...
            logger.fdebug(f"Issue working with zip file.  Likely a broken archive.")
            return {'status': False, 'type' : 'ZIP', 'quality': 'Error processing zip compressed file.'}

        return {'status': True, 'type' : 'ZIP', 'quality': 'Good condition.'}
    elif header.startswith(magic_numbers['RAR']):
        try:
            with rarfile.RarFile(filePath, mode='r') as rarf:
                test_result = rarf.testrar()
                if test_result is not None:
                    return {'status': False, 'type' : 'RAR', 'quality': f'CRC error in file {test_result}.'}
        except:
            logger.fdebug(f"Issue working with rar file.  Likely a broken archive.")
            return {'status': False, 'type' : 'RAR', 'quality': 'Error processing rar compressed file.'}
