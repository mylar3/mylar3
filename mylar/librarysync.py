...
def libraryScan(dir=None, append=False, ComicID=None, ComicName=None, cron=None, queue=None):
    ...
    logger.info('However, ' + str(cbz_retry) + ' out of the ' + str(
        len(failure_list)) + ' files are in a cbz format, which may contain metadata.')
    logger.info('[ERRORS] I have encountered ' + str(
        len(utter_failure_list)) + ' file-scanning errors during the scan, but have recorded the necessary information.')
    ...
    for i in comic_list:
        mylar.IMPORT_STATUS = '[' + str(cnt) + '/' + str(
            comiccnt) + '] Now parsing individual filenames for metadata if available'
            ...
            try:
                ...
                if urllink:
                    ...
                    if match:
                        ...
                        logger.info(
                            'CVINFO file located within directory. Attaching everything in directory that is valid to ComicID: ' + str(
                                cvinfo_CID))
                    ...
                    if issueinfo is None or issueinfo['metadata'] is None:
                        ...
                        if issueinfo is not None:
                            ...
                            try:
                                ...
                                if all([issuevolume is not None, issuevolume != 'None',
                            ...
                            if any([comicname is None, comicname == 'None', issue_number is None,
                                    issue_number == 'None']):
                                logger.fdebug(
                                ...
                                if issuenotes is not None and issuenotes != 'None':
                                    ...
                                    if len(re.findall(regex, issuenotes)) > 0:
                                        ...
                                        try:
                                            logger.info(
                                                '[IMPORT-CBZ] Successfully retrieved CV IssueID for ' + comicname + ' #' + issue_number + ' [' + str(
                                                    issuenotes_id) + ']')
                                        except Exception as ex:
                                ...
                                if cvinfo_CID is None:
                                    ...
                                    if issuenotes_id is None:
                                        logger.info(
                                            '[IMPORT-CBZ] No ComicID detected where it should be. Bypassing this metadata entry and going the parsing route [' + comfilename + ']')
                                    else:
    ...
    if len(watch_kchoice) > 0:
        ...
        logger.info("I have found " + str(watchfound) + " out of " + str(
            comiccnt) + " comics for series that are being watched.")
        wat = 0
        comicids = []
            ...
            if mylar.CONFIG.IMP_MOVE:
                ...
                while (wat < watchfound):
                    ...
                    if mylar.CONFIG.IMP_RENAME:
                        logger.fdebug(
                            'Renaming files according to configuration details : ' + str(mylar.CONFIG.FILE_FORMAT))
                ...
                while (wat < watchfound):
                    ...
                    issuechk = myDB.selectone("SELECT * from issues where ComicID=? AND INT_IssueNumber=?",
...
def scanLibrary(scan=None, queue=None):
    ...
    if scan:
        ...
        if soma == "Completed":
            ...
            logger.info('[IMPORT] Attempting to import ' + str(
                int(soma['import_cv_ids'] + soma['import_count'])) + ' files into your watchlist.')
            ...
            logger.info('[IMPORT-BREAKDOWN] Files that caused errors during the import: ' + str(
                len(soma['utter_failure_list'])))
