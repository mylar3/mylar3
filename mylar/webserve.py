...
#        return serve_template(templatename="reorder.html", title="ReoRdered!", reorder=request)
    ...
    def GenerateCBLReadingList(self, StoryArcID=None):
        ...
        if StoryArcID:
            ReadingArc = myDB.select("SELECT * FROM storyarcs WHERE StoryArcID=? AND (Manual IS NOT 'deleted' OR Manual IS NULL) ORDER BY ReadingOrder", [StoryArcID])
                ...
                with open(fullPath, 'w') as file:
                    ...
                    for issue in ReadingArc:
                        ...
                        if issue['Location'] is not None:
                            filePath = issue['Location'].split(os.path.sep)
